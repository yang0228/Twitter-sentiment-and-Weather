...
/**
 * Class implemetinga GeoMesa feature store to hold Tweets
 * 
 * @author lmorandini, QingyangHong
 *
 */
public class TweetFeatureStore {

  protected static String schemaDef = "location:Point:srid=4326,id:String,timestamp:String,text:String,tokens:String,sentiment:String";

  public TweetFeatureStore() {
    // TODO Auto-generated constructor stub
  }

  /**
   * Returns a SimpleFeatureType to hold Tweets with tableName as name of table
   * 
   * @param tableName
  ...
  public static SimpleFeatureType createSchema(String tableName)
      throws SchemaException {
    return DataUtilities.createType(tableName, TweetFeatureStore.schemaDef);
  }
  ...
  public static void createFeatureType(GeoMesaOptions options)
    ...
    // Creates the FeatureType if not existing
    if (dataStore.getSchema(options.tableName) == null) {
      dataStore.createSchema(TweetFeatureStore.createSchema(options.tableName));
    }
  }
}
